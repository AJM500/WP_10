...
let textbox = {
  ...
  index: 0,
  text: [
    `${playerName}야(아), 기다리고 있었단다`,
    `이곳은 물타입 포켓몬이 풍부한 도시란다.`,
    `이곳의 트레이너들은 물타입 포켓몬을 주로 파트너로 둔다고 하지.`,
    `너가 잃어버린 풀타입 포켓몬이 이 도시 어딘가에 있을 거란다.`,
    '저번 도시와 마찬가지로',
    `포켓몬을 찾는다면 이 도시에서의 포켓몬 배틀에 큰 도움이 되겠지.`,
    `이 도시의 챔피언이 되고 싶다면 파란색 건물을 먼저 찾아가보려무나.`
  ]
};
...
function isBlocked(x, y, width, height) {
  ...
  return WatermapData.blockedZones.some(zone =>
    hbX < zone.x + zone.w &&
    hbX + hbW > zone.x &&
    hbY < zone.y + zone.h &&
    hbY + hbH > zone.y
...
function moveStage(){
  ...
  if(sessionStorage.getItem('STAGE') == 'CLEAR_1'){
    ...
    if(player.x == 426 && player.y == 420){
      keys['ArrowDown'] = true;
    }else if(player.x == 426 && player.y == 434){
      keys['ArrowDown'] = false;
      keys['ArrowLeft'] = true;
    }else if(player.x == 230 && player.y == 434){
      keys['ArrowLeft'] = false;
      keys['ArrowUp'] = true;
    }else if(player.x == 230 && player.y == 258){
      keys['ArrowUp'] = false;
    ...
    if(player.x == 230 && player.y == 258){
      keys['ArrowRight'] = true;
    }else if(player.x == 788 && player.y == 258){
      keys['ArrowRight'] = false;
      keys['ArrowUp'] = true;
    }else if(player.x == 788 && player.y == 256){
      keys['ArrowUp'] = false;
...
function triggerPokemonCutscene() {
  ...
  const handleKeydown = (e) => {
    if (e.key.toLowerCase() === 'e') {
      ...
      //  풀속성 포켓몬 획득 여부 저장
      sessionStorage.setItem("grasspocketmon", "true");
